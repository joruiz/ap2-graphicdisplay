...
class GraphicDisplay
{
	public:
		...
		void begin(uint8_t address, uint8_t _buttonsChangePin);
		uint8_t getVersion();
		boolean isReady();
		uint8_t clear();
		uint8_t drawLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint16_t colour);
		uint8_t drawRectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint16_t colour);
		uint8_t drawCircle(uint8_t x1, uint8_t y1, uint8_t radious, uint16_t colour);
		uint8_t drawFillCircle(uint8_t x1, uint8_t y1, uint8_t radious, uint16_t colour);
		uint8_t drawImage(uint16_t dirH, uint16_t dirL, uint8_t x, uint8_t y);
		uint8_t printText(String text, uint8_t line, uint8_t column, uint16_t colour);
		boolean buttonsChanged();
		uint8_t getButtons();

	private:
		uint8_t _address;
		uint8_t _buttonsChangePin;
};
